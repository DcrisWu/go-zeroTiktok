...
func (m *defaultVideoModel) Insert(ctx context.Context, data *Video) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Id, data.AuthorId, data.PlayUrl, data.CoverUrl, data.FavoriteCount, data.CommentCount, data.Title, data.DeletedAt)
	return ret, err
}
...
func (m *defaultVideoModel) Update(ctx context.Context, data *Video) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.AuthorId, data.PlayUrl, data.CoverUrl, data.FavoriteCount, data.CommentCount, data.Title, data.DeletedAt, data.Id)
	return err
}
